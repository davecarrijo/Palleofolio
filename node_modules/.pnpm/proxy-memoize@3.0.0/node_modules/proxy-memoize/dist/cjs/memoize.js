...
const untrack = (x, seen) => {
    if (!isObject(x))
        return x;
    const originalObj = (0, proxy_compare_1.getUntracked)(x);
    if (originalObj !== null) {
        (0, proxy_compare_1.trackMemo)(x);
...
const touchAffected = (dst, src, affected) => {
    ...
    if (!isObject(dst) || !isObject(src))
        return;
    if (trackMemoOriginalObjSet.has((0, proxy_compare_1.getUntracked)(src))) {
        (0, proxy_compare_1.trackMemo)(dst);
        return;
    }
    const used = affected.get((0, proxy_compare_1.getUntracked)(src) || src);
    if (!used)
        return;
...
const isOriginalEqual = (x, y) => {
    for (let xx = x; xx; x = xx, xx = (0, proxy_compare_1.getUntracked)(xx))
        ;
    for (let yy = y; yy; y = yy, yy = (0, proxy_compare_1.getUntracked)(yy))
        ;
...
function memoize(fn, options) {
    ...
    const memoizedFn = (obj) => {
        ...
        for (let i = 0; i < size; i += 1) {
            ...
            if (!(0, proxy_compare_1.isChanged)(memoObj, obj, memoAffected, new WeakMap(), isOriginalEqual)) {
        ...
        const proxy = (0, proxy_compare_1.createProxy)(obj, affected, undefined, targetCache);
